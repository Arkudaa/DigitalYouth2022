...
public class PlayerMove : MonoBehaviour 
{
	...
	//Q- Additions
	private bool P_Moving = false;	//Are we pressing movement buttons?
	...
	void FixedUpdate() {
		...
		if (animator) {
			...
			animator.SetBool ("P_Moving", P_Moving);	//Set the boolean
			animator.SetFloat ("Vertical_Para", v);		//Set the float in the animator control to match the input.
			animator.SetFloat ("Horizontal_Para", h);
		}
	}
	...
	public void ForceIdle() {
		animator.SetFloat("Horizontal_Para",0);
		animator.SetFloat("Vertical_Para",0);
		animator.SetBool("Grounded", true);
		animator.SetFloat("DistanceToTarget", 0.0f);
